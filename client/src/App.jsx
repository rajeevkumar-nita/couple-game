...
function App() {
  ...
  const bgClass =
    activeCurse === "MIRROR"
      ...
      : "bg-gray-900";
  const bannerColor =
    activeCurse === "MIRROR" ? "bg-red-600" : "bg-blue-600";
  const borderClass =
    activeCurse === "MIRROR"
          ...
          <div className="text-left">
            <h2
              className={`text-lg font-bold ${
                role === "walker" ? "text-blue-400" : "text-green-400"
              }`}
            >
              {role === "walker" ? "🚶 WALKER" : "👀 WATCHER"}
            </h2>
          ...
          <div className="flex items-center gap-3">
            <div
              className={`text-3xl transition-all ${
                hasKey ? "opacity-100 scale-125" : "opacity-20 grayscale"
              }`}
            >
              🔑
            </div>
            <div
              className={`text-2xl font-mono font-bold border-2 px-3 rounded ${
                timeLeft < 10
        ...
        <>
          ...
          {role === "walker" && !winner && !gameOverMsg && (
            <div className="mt-6 grid grid-cols-3 gap-2 w-48">
              <div></div>
              <button
                className={`p-4 rounded-lg active:bg-blue-600 shadow-lg text-2xl ${
                  activeCurse === "MIRROR"
                    ? "bg-red-600"
                    : activeCurse === "SPEED"
                    ? "bg-blue-600"
