...
function App() {
  ...
  const handleShare = () => {
    ...
    const text = `❤️ Sync Hearts Score: ${gameReport.score}`;
    if (navigator.share) navigator.share({ title: 'Sync Hearts', text: text }).catch(console.error);
    else alert("Score copied: " + text);
  };
  ...
  useEffect(() => {
    ...
    socket.on("start_game", (data) => {
      setGameStarted(true); setMap(data.map); setPlayerPos(data.startPos);
      setCurrentLevel(data.level); setActiveCurse(null); setHasKey(false); setGameReport(null);
    });
    ...
    socket.on("curse_triggered", (data) => { 
      setActiveCurse(data.type); 
      if (data.type === 'MIRROR') AUDIO_CURSE.play();
      if (data.type === 'SPEED') AUDIO_SPEED.play();
    });
    socket.on("curse_ended", () => setActiveCurse(null));

    socket.on("game_won", (report) => { setWinner(true); setGameReport(report); AUDIO_WIN.play(); });
    socket.on("game_over", (data) => { setGameOverMsg(data.msg); setGameReport(data.report); AUDIO_LOSE.play(); });
    socket.on("reset_game_state", () => { setWinner(false); setGameOverMsg(""); setActiveCurse(null); setHasKey(false); setGameReport(null); });
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-screen text-white select-none relative overflow-hidden pb-10 transition-colors duration-500 ${bgClass}`}>
      ...
      {/* --- VOICE CHAT INTEGRATED HERE --- */}
      {gameStarted && (
        <VoiceChat socket={socket} roomCode={room} />
      )}
      ...
      <div className="mt-4 mb-2 text-center px-4 w-full max-w-lg">
        <div className="flex justify-between items-center mb-2">
           <div className="text-left">
             ...
             <p className="text-yellow-400 font-bold text-sm">⭐ LVL {currentLevel}</p>
           </div>
           <div className="flex items-center gap-3">
             <div className={`text-3xl transition-all ${hasKey ? 'opacity-100 scale-125' : 'opacity-20 grayscale'}`}>🔑</div>
             <div className={`text-2xl font-mono font-bold border-2 px-3 rounded ${timeLeft < 10 ? 'text-red-500 border-red-500 animate-pulse' : 'text-white border-gray-500'}`}>⏳ {timeLeft}s</div>
      ...
      {(winner || gameOverMsg) && gameReport && (
        <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center z-50 backdrop-blur-md px-4">
          <div className="bg-gray-800 p-6 rounded-2xl border-4 border-purple-500 text-center shadow-2xl w-full max-w-sm relative overflow-hidden">
            <h2 className="text-2xl font-bold text-white mb-4 uppercase tracking-wider">
              {winner ? "🎉 MISSION COMPLETE" : "💀 MISSION FAILED"}
            </h2>
            <div className="bg-gray-900 p-4 rounded-lg mb-6 text-left space-y-3">
              <div className="flex justify-between border-b border-gray-700 pb-2">
                <span className="text-gray-400">Sync Score</span>
                <span className="text-green-400 font-mono font-bold text-xl">{gameReport.score}</span>
              </div>
              <div className="flex justify-between border-b border-gray-700 pb-2">
                <span className="text-gray-400">Trust Factor</span>
                <span className="text-yellow-400 font-bold">{gameReport.trust}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Comm Style</span>
                <span className="text-blue-400 font-bold">{gameReport.comms}</span>
              </div>
            </div>
            <div className="space-y-3">
              <button onClick={handleShare} className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded-lg">📤 Share Result</button>
              {winner ? (
                <button onClick={handleNextLevel} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg">🚀 Next Level</button>
              ) : (
                <button onClick={handleRestart} className="w-full bg-pink-600 hover:bg-pink-700 text-white font-bold py-3 rounded-lg">🔄 Retry Level {currentLevel}</button>
              )}
            </div>
          </div>
        </div>
